...
import com.sa.server.util.FileUtil;
import com.sa.server.util.Util;
...
public class Base64RequestBody extends RequestBody {
    ...
    public void writeTo(BufferedSink sink) throws IOException {
        ...
        if (stringParams != null && stringParams.size() > 0) {
            String jsonStr = JSON.toJSONString(stringParams);
        ...
        if (fileMap != null && fileMap.size() > 0) {
            ...
            for (String k : fileMap.keySet()) {
                ...
                byte[] base = FileUtil.bytesToBase64(buf).getBytes();
